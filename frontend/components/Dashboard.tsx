...
import { getEvents, getPrediction, getRecommendation, getSimilarEvents, Event, Prediction, Recommendation, RecalibrationLogEntry, OutcomeSubmissionResult, SimilarEvent } from "../lib/api";
import PredictionPanel from "./PredictionPanel";
import RecommendationPanel from "./RecommendationPanel";
import SimilarEventsPanel from "./SimilarEventsPanel";
import OutcomeForm from "./OutcomeForm";
import RecalibrationViz from "./RecalibrationViz";
...
export default function Dashboard() {
  ...
  const [similarEvents, setSimilarEvents] = useState<SimilarEvent[] | null>(null);
  ...
  const handleEventSelect = async (eventId: string) => {
    ...
    try {
      const [pred, rec, sim] = await Promise.all([
      ...
      setSimilarEvents(sim);
    } catch (err: any) {
      setErrorDetails(err.message || "Failed to fetch details/similar events from backend.");
    } finally {
  ...
  const handleOutcomeSubmitted = (result: OutcomeSubmissionResult) => {
    ...
    setLatestOutcomeUpdate(result.recalibration);
    
    // 2. Refetch prediction/recommendation/similar details silently
    if (selectedEventId) {
      Promise.all([
        getPrediction(selectedEventId),
        getRecommendation(selectedEventId),
        getSimilarEvents(selectedEventId)
      ]).then(([pred, rec, sim]) => {
        setPrediction(pred);
        setRecommendation(rec);
        setSimilarEvents(sim);
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-slate-900 text-slate-100 font-sans">
      ...
      <div className="w-[25%] h-full border-r border-slate-700 bg-slate-950 flex flex-col flex-shrink-0">
        ...
        {/* Header Branding */}
        <div className="p-4 border-b border-slate-700 bg-slate-900 flex items-center justify-between gap-2 flex-shrink-0">
                    ...
                    <button
                      ...
                      className={`text-left p-3 rounded border text-[11px] transition-all duration-150 ${
                        isSelected 
                          ...
                          : "bg-slate-900 border-slate-850 text-slate-400 hover:bg-slate-850 hover:border-slate-700"
                      }`}
                    >
